...
def choose(paragraphs, select, k):
    ...
    new = [item for item in paragraphs if select(item)== True]
...
def about(topic):
    ...
    def select (paragraph):
        new = remove_punctuation(paragraph).lower().split()
        for i in topic:
            if i in new:
                return True
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == True and len(reference_words) == False:
        return 0.0
    elif len(typed_words) == False and len(reference_words) == False: 
        return 100.0
    elif not len(typed_words) and len(reference_words): 
        return 0.0
    else:
    ...
    answer = correct/len(typed_words)*100.0
    print("DEBUG: ", correct )
    return answer
